...
qt-misc-anki-updatedanki-has-been-released = <h1>Kelma Updated</h1>Kelma { $val } has been released.<br><br>
qt-misc-automatic-syncing-and-backups-have-been = Backup successfully restored. Automatic syncing and backups have been disabled for now. To enable them again, close the profile or restart Kelma.
...
qt-misc-unable-to-record =
    Unable to record. Please ensure a microphone is connected, and Kelma has permission to use the microphone.
    If other programs are using your microphone, closing them may help.
    
    Original error: { $error }
qt-misc-please-ensure-a-profile-is-open = Please ensure a profile is open and Kelma is not busy, then try again.
...
qt-misc-unable-to-access-anki-media-folder = Unable to access Kelma media folder. The permissions on your system's temporary folder may be incorrect.
...
qt-misc-your-collection-file-appears-to-be = Your collection file appears to be corrupt. This can happen when the file is copied or moved while Kelma is open, or when the collection is stored on a network or cloud drive. If problems persist after restarting your computer, please open an automatic backup from the profile screen.
...
qt-misc-your-firewall-or-antivirus-program-is = Your firewall or antivirus program is preventing Kelma from creating a connection to itself. Please add an exception for Kelma.
qt-misc-error = Error
...
qt-misc-incompatible-video-driver = Your video driver is incompatible. Please start Kelma again, and Kelma will switch to a slower, more compatible mode.
qt-misc-error-loading-graphics-driver = Error loading '{ $mode }' graphics driver. Please start Kelma again to try the next driver. { $context }
qt-misc-anki-is-running = Kelma Already Running
qt-misc-if-instance-is-not-responding = If the existing instance of Kelma is not responding, please close it using your task manager, or restart your computer.
qt-misc-second =
    { $count ->
        [one] { $count } second
       *[other] { $count } seconds
