...
from .v2_client import V2Client, V2Conflict
from . import v2_sync
...
def build_plan(col: Collection, client: V2Client, since: str | None = None) -> SyncPlan:
    ...
    """
    local = v2_sync.local_manifest(col)
...
def push_local(col: Collection, client: V2Client, item: DiffItem, *, force: bool = False) -> dict[str, Any] | None:
    ...
    if item.resource == "notes":
        rec = v2_sync.note_record(col, item.key)
        if not rec:
            return None
    ...
    if item.resource == "cards":
        rec = v2_sync.card_record(col, int(item.key))
        if not rec:
            return None
    ...
    if item.resource == "notetypes":
        rec = v2_sync.notetype_record(col, int(item.key))
        if not rec:
            return None
        base = (item.server or {}).get("checksum", "") if item.status != "local-only" else ""
    ...
    if item.resource == "decks":
        rec = v2_sync.deck_record(col, item.key)
        if not rec:
            return None
        base = (item.server or {}).get("checksum", "") if item.status != "local-only" else ""
